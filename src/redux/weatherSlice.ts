...
>('weather/fetchWeatherByCity', async (city, { rejectWithValue }) => {
    try {
        const weather = await WeatherApi.fetchWeatherByCity(city.name);
        return { city, weather };
    } catch (error) {
...
export const fetchHourlyForecast = createAsyncThunk<
    ...
    City,
    { rejectValue: WeatherErrorResponse }
>('weather/fetchHourlyForecast', async (city, { rejectWithValue }) => {
    try {
        const forecast = await WeatherApi.fetchHourlyForecast(city.name);
...
export const fetchWeatherByCityId = createAsyncThunk<
    ...
    string,
    { rejectValue: WeatherErrorResponse }
>('weather/fetchWeatherByCityId', async (cityId, { rejectWithValue }) => {
    try {
        const weather = await WeatherApi.fetchWeatherByCityId(cityId);
...
>(
    ...
    async (cityId, { rejectWithValue }) => {
        try {
            const forecast = await WeatherApi.fetchHourlyForecastByCityId(
                cityId
            );
...
export const { addCity, removeCity, loadSavedCities } = weatherSlice.actions;
